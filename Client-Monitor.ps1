...
Function Send-Email() {
	...
	if($DebugVerbosity -ge 3) {
		Write-Debug -Message "{Send-Email} parameters:" -Threshold 3 -Prefix '>>>>'
		Write-Debug -Message "`$RELAYSERVER = $RELAYSERVER" -Threshold 3 -Prefix '>>>>>>'
		Write-Debug -Message "`$RELAYPORT = $RELAYPORT" -Threshold 3 -Prefix '>>>>>>'
		Write-Debug -Message "`$FROM = $FROM" -Threshold 3 -Prefix '>>>>>>'
		Write-Debug -Message "`$TO = $TO" -Threshold 3 -Prefix '>>>>>>'
		Write-Debug -Message "`$SUBJECT = $SUBJECT" -Threshold 3 -Prefix '>>>>>>'
		Write-Debug -Message "`$BODY = $BODY" -Threshold 3 -Prefix '>>>>>>'
	}
	...
	if($NotificationsAsHTML -eq $True) {
		...
		Write-Debug -Message "Email is HTML-formatted, inject body text into the message." -Threshold 2 -Prefix '>>>>'
	} else {
		# WARNING!!!!!!!!!
...
Function Mount-UserHives() {
	...
	if($Dismount -eq $True) {
		...
		$returnedHives = Invoke-Command @invokeParams -ArgumentList $Info,${function:Write-Debug},$DebugVerbosity -ScriptBlock {
			...
			#Write-Debug -Message 'Profiles on the target machine:' -Threshold 1 -Prefix '>>'
